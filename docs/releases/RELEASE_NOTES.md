...
A modernização foi validada, na data deste registro, com 31 testes aprovados, Clippy estrito, builds release Linux/Windows GNU, `cargo audit`, `cargo deny check` e smoke tests reais da interface e das operações de arquivo. A suíte atual evoluiu posteriormente para 45 testes pelo harness, com 43 aprovados e 2 ignorados explicitamente; consulte [`../reference/testing.md`](../reference/testing.md) para o estado corrente. A execução nativa identificada em Windows 10 22H2, testes de DPI, manifesto PE, UNC/SMB, reparse points e paths longos permanecem como gates de compatibilidade posteriores.
