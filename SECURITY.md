...
A UI envia o trabalho de listagem a um worker e atualiza o modelo Slint somente pelo event loop. Cada carregamento recebe uma geração; quando uma navegação mais recente ocorre, o resultado anterior é descartado antes de alterar caminho, status ou linhas. Falhas ao iniciar o worker e falhas internas de atualização viram mensagens controladas, não panic.
