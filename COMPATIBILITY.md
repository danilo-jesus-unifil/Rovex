...
O lote de preview decodifica imagens estáticas suportadas diretamente pelo crate `image` 0.25.10 — BMP, GIF, JPEG, PNG e WebP — com formato inferido pelo conteúdo, não apenas pela extensão. O núcleo rejeita symlink/reparse point, exige arquivo regular, limita entrada a 128 MiB, dimensões a 8192×8192 e alocação de decode a 64 MiB, depois reduz a imagem para no máximo 256px por lado. O mesmo worker agora tenta, somente como fallback literal, texto UTF-8 sem BOM, UTF-8 com BOM e UTF-16LE/BE com BOM; a leitura textual é limitada a 64 KiB, remove BOM da apresentação, rejeita NUL/controles binários e bytes inválidos, e marca conteúdo truncado. O worker dedicado coalesce requests, cancela gerações antigas e mantém cache LRU limitado a 128 entradas/32 MiB; falhas, formatos falsos e conteúdo corrompido mostram fallback textual/ícone genérico sem derrubar a UI. A seleção de um único arquivo abre painel escuro contido; seleção múltipla, pastas e navegação escondem/cancelam a prévia. `scripts/test_preview.sh` confirmou PNG válido, `.jpg` inválido, texto UTF-8 com BOM/Unicode, fechamento do painel e processo ativo; testes unitários cobrem limites, endianness UTF-16, binário e symlink. O preview não renderiza HTML/Markdown, não cria links e não executa conteúdo. PDF, vídeo, áudio, Office, codecs externos, handlers COM/Explorer e preview nativo Windows permanecem deliberadamente fora deste lote e exigem workers/processos isolados posteriores.
...
No alvo Windows, `delete_entry` valida a origem e usa `IFileOperation`/COM com `IShellItem` e `FOFX_RECYCLEONDELETE` para enviar arquivos, symlinks e diretórios vazios à Lixeira, sem chamar comandos externos e sem fallback silencioso para exclusão permanente. `FOFX_EARLYFAILURE`, `FOF_NORECURSION` e uma leitura prévia preservam o contrato de que diretórios não vazios não são removidos. A confirmação continua na UI, o Shell não abre uma confirmação duplicada e HRESULTs/aborto do Shell são publicados como falha, preservando o item. Se COM, o parsing do item ou o enfileiramento falharem antes de `PerformOperations`, o adapter tenta `SHFileOperationW` com `FOF_ALLOWUNDO`; falhas depois do início não são repetidas. No Linux de desenvolvimento, o fallback anterior de remoção permanente permanece para que os testes de operações continuem determinísticos.

O cross-check Windows GNU compila o adapter, os bindings UTF-16 e a mensagem específica de Lixeira; o runner `windows-latest` executa a suíte nativa e o smoke CLI, mas não fixa o build do Windows nem valida visualmente a restauração. A matriz interativa ainda deve validar uma Lixeira real, restauração no Explorer, paths Unicode/longos, UNC/SMB, volumes removíveis, ACLs, arquivos em uso, políticas de grupo e comportamento quando a Lixeira estiver desabilitada. `IFileOperation`/COM atende o primeiro passo de integração nativa, mas não transforma o cross-build ou o CI não interativo em prova completa de compatibilidade Windows 10/11.

## Terminal contextual

A ação `Abrir Terminal aqui` é explícita e só aparece habilitada no alvo Windows. Uma pasta é usada diretamente; para um arquivo, o diretório pai é derivado sem canonicalização que altere o nome apresentado. O worker valida caminho absoluto existente e tenta, nesta ordem, `wt.exe -w new new-tab --startingDirectory <path>`, `powershell.exe -NoLogo -NoExit` e `cmd.exe /D /K`; o caminho também é passado por `current_dir`. Cada argumento fica separado no `Command`, sem `cmd /c`, `powershell -Command`, `start` ou concatenação de input. O spawn não espera o terminal fechar e o resultado volta ao event loop.
